...
class SpoolmanAPI:
    ...
    def get_active_spool_id(self) -> int:
        ...
        if not result or "result" not in result or "spool_id" not in result["result"]:
            return None
        return result["result"]["spool_id"]
    ...
    def update_spool_weight(self, spool_id: int, remaining_weight: float) -> bool:
        """Updates the remaining weight of a spool."""
        try:
            result = self.api.post_request("server/spoolman/proxy", json={
            ...
            if not result or "result" not in result:
                logging.warning(f"Error updating spool weight: {result}")
                return False
            return True
        except Exception as e:
            logging.error(f"Error updating spool weight: {e}")
            return False
