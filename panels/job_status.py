...
#Begin VSYS
            if not self.macro_adaptatif_mesh:
                self._screen._ws.klippy.print_start(self.filename, 0)
            else:
                self._screen._ws.klippy.print_start(self.filename, 3)
#End VSYS
            self.new_print()
        else:
        ...
        if self.state == "printing":
            ...
            if self.zoffset != 0:
                if "Z_OFFSET_APPLY_ENDSTOP" in self._printer.available_commands:
                    self.buttons['button_grid'].attach(Gtk.Label(), 0, 0, 1, 1) #VSYS
                else:
                    self.buttons['button_grid'].attach(Gtk.Label(), 0, 0, 1, 1)
                if "Z_OFFSET_APPLY_PROBE" in self._printer.available_commands:
                    self.buttons['button_grid'].attach(Gtk.Label(), 1, 0, 1, 1) #VSYS
                else:
    ...
    def show_file_thumbnail(self):
        ...
        if width <= 1 or height <= 1:
            width = max_width
            height = max_height
        # self.labels['thumbnail'].set_hexpand(False) #VSYS
    ...
    def animate_label(self):
        ...
        if ellipsized:
            ...
            self.labels['file'].set_label(self.filename_label['current'] + " " * 5)
        else:
    ...
    def update_file_metadata(self, response=False):
        if self._files.file_metadata_exists(self.filename):
            ...
            if "job_id" in self.file_metadata and self.file_metadata['job_id']:
                ...
                if history and history['result'] and history['result']['job']['status'] == "completed" and history['result']['job']['print_duration']:
                    self.file_metadata["last_time"] = history['result']['job']['print_duration']
        elif not response:
