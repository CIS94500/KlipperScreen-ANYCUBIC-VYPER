...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        pixbuf_renderer = Gtk.CellRendererPixbuf(xpad=25, ypad=25)
        ...
        self._treeview.connect("button-press-event", self._on_icon_clicked)
        self._column_icon = column_icon  # garder une référence
    ...
    def _on_icon_clicked(self, treeview, event):
        ...
        if column == self._column_icon:
            model = treeview.get_model()
            # Convertir le path du modèle trié vers le modèle de base
            child_path = model.convert_path_to_child_path(path)
            spool = model.get_model()[child_path][0]
            self._screen.show_panel("spool_edit", "Edit Spool", spool=spool)
        return False
