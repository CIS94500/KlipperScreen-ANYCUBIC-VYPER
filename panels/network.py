...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.show_add = False
        self._screen.base_panel.update_wifi_icon()
        self._wlan = Gtk.ListStore(str, str)
        ...
        if len(self.wireless_interfaces) <= 1:
            ...
            for iface in self.wireless_interfaces:
                self._wlan.append([iface, iface])
            self.labels['interface'] = ComboBoxPlus(model=self._wlan, hexpand=True)
            renderer = Gtk.CellRendererText()
            self.labels['interface'].pack_start(renderer, True)
            self.labels['interface'].add_attribute(renderer, "text", 0)
            self.labels['interface'].connect("changed", self.iface_changed)
            self.labels['interface'].connect("notify::popup-shown", self.on_popup_shown)
            if self.interface is not None:
                for i, row in enumerate(self._wlan):
                    if row[0] == self.interface:
                        self.labels['interface'].set_active(i)
                        break
    ...
    def iface_changed(self, combo):
        ...
        selected_iface = model[tree_iter][0]
    ...
    def on_popup_shown(self, combo_box, params):
        if combo_box.get_property("popup-shown"):
            ...
            if duration < 0.5:
                GLib.idle_add(combo_box.popup, priority=GLib.PRIORITY_HIGH_IDLE)
