...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        if self.ks_printer_cfg is not None:
            bs = self.ks_printer_cfg.get("z_babystep_values", 'None')
            if re.match(r'^[0-9,\.\s]+$', bs):
                bs = [str(i.strip()) for i in bs.split(',')]
                if 1 < len(bs) < 3:
                    self.z_deltas = bs
                    self.z_delta = float(self.z_deltas[-1])

        zgrid = Gtk.Grid()
        for j, i in enumerate(self.z_deltas):
            x = float(i)
            self.labels[f"zdelta{x}"] = self._gtk.Button(label=i)
            self.labels[f"zdelta{x}"].connect("clicked", self.change_percent_delta, "z_offset", x)
            ctx = self.labels[f"zdelta{x}"].get_style_context()
            if (self._screen.lang_ltr and j == 0) or (not self._screen.lang_ltr and j == len(self.z_deltas) - 1):
                ctx.add_class("distbutton_top")
            elif (not self._screen.lang_ltr and j == 0) or (self._screen.lang_ltr and j == len(self.z_deltas) - 1):
                ctx.add_class("distbutton_bottom")
            else:
                ctx.add_class("distbutton")
            if x == float(self.z_delta):
                ctx.add_class("distbutton_active")
            zgrid.attach(self.labels[f"zdelta{x}"], j, 0, 1, 1)

        spdgrid = Gtk.Grid()
        for j, i in enumerate(self.speed_deltas):
            x = int(i)
            self.labels[f"sdelta{x}"] = self._gtk.Button(label=f"{i}%")
            self.labels[f"sdelta{x}"].connect("clicked", self.change_percent_delta, "speed", x)
            ctx = self.labels[f"sdelta{x}"].get_style_context()
            if (self._screen.lang_ltr and j == 0) or (not self._screen.lang_ltr and j == len(self.speed_deltas) - 1):
                ctx.add_class("distbutton_top")
            elif (not self._screen.lang_ltr and j == 0) or (self._screen.lang_ltr and j == len(self.speed_deltas) - 1):
                ctx.add_class("distbutton_bottom")
            else:
                ctx.add_class("distbutton")
            if x == int(self.s_delta):
                ctx.add_class("distbutton_active")
            spdgrid.attach(self.labels[f"sdelta{x}"], j, 0, 1, 1)

        extgrid = Gtk.Grid()
        for j, i in enumerate(self.extrude_deltas):
            x = int(i)
            self.labels[f"edelta{x}"] = self._gtk.Button(label=f"{i}%")
            self.labels[f"edelta{x}"].connect("clicked", self.change_percent_delta, "extrude", x)
            ctx = self.labels[f"edelta{x}"].get_style_context()
            if (self._screen.lang_ltr and j == 0) or (not self._screen.lang_ltr and j == len(self.extrude_deltas) - 1):
                ctx.add_class("distbutton_top")
            elif (not self._screen.lang_ltr and j == 0) or (self._screen.lang_ltr and j == len(self.extrude_deltas) - 1):
                ctx.add_class("distbutton_bottom")
            else:
                ctx.add_class("distbutton")
        ...
        self.labels['zoffset'] = self._gtk.Button("refresh", '  0.000' + _("mm"),
    ...
    def change_babystepping(self, widget, direction):
        if direction == "reset":
            self.labels['zoffset'].set_label('  0.000mm')
